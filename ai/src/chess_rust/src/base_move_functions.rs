...
fn generate_pawn_moves_black(board: &Board, row: usize, col: usize) -> Vec<i32> {
    ...
    if col > 0 {
        if board.full_board[row + 1][col - 1].team == Team::W {
            moves.push((row + 1) * 10 + col - 1 as i32);
        }
        if row == 4
            && board.full_board[4][col - 1].team == Team::W
            && board.full_board[4][col - 1].kind == Kind::Pawn
        {
            if board.white_prime % primes(col - 1) == 0
                && board.black_prime1 % primes1(board.black_i_to_p.get(row + col)) == 0
            {
                let ep = 20 + col - 1 as i32;
                moves.push(ep);
            }
        }
    }
...
pub fn generate_pawn_moves_white(board: &Board, row: usize, col: usize) -> Vec<i32> {
    let mut moves = Vec::new();
    ...
    if col > 0 {
        if board.full_board[row - 1][col - 1].team == Team::B {
            moves.push((row - 1) * 10 + col as i32 - 1);
        }
        if row == 3
            && board.full_board[3][col - 1].team == Team::B
            && board.full_board[3][col - 1].kind == Kind::Pawn
        {
            if board.black_prime % primes(col - 1) == 0
                && board.white_prime1 % primes1(board.white_i_to_p.get(row + col)) == 0
            {
                let ep = 20 + col - 1 as i32;
                moves.push(ep);
            }
...
pub fn generate_knight_moves(board: &Board, row: usize, col: usize) -> Vec<i32> {
    let mut moves = Vec::new();
    ...
    for (i, j) in knight_moves.iter() {
        ...
        if move_row >= 0 && move_row < 8 && move_col >= 0 && move_col < 8 {
            ...
            if board.full_board[move_row][move_col].team != team {
                moves.push(move_row * 10 + move_col as i32);
            }
        }
    }
...
pub fn generate_bishop_moves(board: &Board, row: usize, col: usize) -> Vec<i32> {
    let mut moves = Vec::new();
    let team = board.full_board[row][col].team;
    ...
    for (dir_row, dir_col) in directions.iter() {
        ...
        while move_row >= 0 && move_row < 8 && move_col >= 0 && move_col < 8 {
            ...
            moves.push(move_row_usize * 10 + move_col_usize as i32);
...
pub fn generate_king_moves(board: &Board, row: usize, col: usize) -> Vec<i32> {
    let mut moves = Vec::new();
    let team = board.full_board[row][col].team;

    // Check diagonals, horizontal, and vertical moves by king
    let offsets = [
        (-1, -1),
        (-1, 0),
        (-1, 1),
...
pub fn generate_rook_moves(board: &Board, row: usize, col: usize) -> Vec<i32> {
    let mut moves = Vec::new();
    let team = board.full_board[row][col].team;
