pub fn all_moves_gen(&mut board:Board){
    ...
    let mut b_ks = board.prime2 % 13 != 0 
        && board.prime2 % 11 != 0 
        && board.full_board[0][6].team == Team::N 
        && board.full_board[0][5].team == Team::N;

    // Castling conditions for Black Queen Side
    let mut b_qs = board.prime2 % 13 != 0 
        && board.prime2 % 7 != 0 
        && board.full_board[0][1].team == Team::N 
        && board.full_board[0][2].team == Team::N 
        && board.full_board[0][3].team == Team::N;

    // Castling conditions for White King Side
    let mut w_ks = board.prime2 % 5 != 0 
        && board.prime2 % 3 != 0 
        && board.full_board[7][6].team == Team::N 
        && board.full_board[7][5].team == Team::N;

    // Castling conditions for White Queen Side
    let mut w_qs = board.prime2 % 5 != 0 
        && board.prime2 % 2 != 0 
        && board.full_board[7][1].team == Team::N 
        && board.full_board[7][2].team == Team::N 
        && board.full_board[7][3].team == Team::N;
    ...
    let b_king_index = board.black_indexes.get(&PieceId::K)
        .expect("Black king not found on board"); // or handle None case more gracefully
    let w_king_index = board.white_indexes.get(&PieceId::K)
        .expect("White king not found on board"); // or handle None case more gracefully
    ...
    for &piece_id in board.white_piece_ids {
        let &piece_index = board.white_indexes.get(&piece_id) 
        ...
        for (index, value)b_king_moves.iter.enumerate{
    ...
    for i in board.black_piece_ids{
        ...
        for (index, value)w_king_moves.iter.enumerate{
            if all_moves.contains(value){
    ...
    if black_checking.length()>0{
        for &i in black_checking{
            ...
            if k==Kind::Rook || k==Kind::Bishop || k==Kind::Queen{
                ...
                direction.push(board.white_indexes.get(PieceId::King)%10-board.black_indexes.get(i)%10)
                in_check_directional(i, Team::Black, direction);
            }
        }
    }
    if white_checking.length()>0{
        for &i in white_checking{
            ...
            if k==Kind::Rook || k==Kind::Bishop || k==Kind::Queen{
                ...
                direction.push(board.black_indexes.get(PieceId::King)%10-board.white_indexes.get(i)%10)
                in_check_directional(i, Team::White, direction);
            }
        }
    }
...
pub fn move(&mut board:Board, move_piece:PieceId, indexes:i32, pawn_premotion_queen:bool){
    ...
    if board.turn%2==0{
        ...
        if new_indexes=99{
            board.full_board[7][4]=Piece{0, PieceId::Empty, Team::None};
            board.full_board[7][7]=Piece{0, PieceId::Empty, Team::None};
            board.full_board[7][6]=Piece{0, PieceId::King, Team::White};
            board.full_board[7][5]=Piece{500, PieceId::Rook, Team::White};
        ...
        if new_indexes=99{
            board.full_board[7][1]=Piece{0, PieceId::Empty, Team::None};
            board.full_board[7][0]=Piece{0, PieceId::Empty, Team::None};
            board.full_board[7][4]=Piece{0, PieceId::Empty, Team::None};
            board.full_board[7][2]=Piece{0, PieceId::King, Team::White};
            board.full_board[7][3]=Piece{500, PieceId::Rook, Team::White};
        ...
        if map_piece_id_to_kind(move_piece)=Kind::Pawn && new_col != old_col && old_points==0{
            ep_white((&mut board, move_piece, indexes));
            return
        }
        ...
        if map_piece_id_to_kind move_piece==Kind::Pawn && row ==0{
            ...
            if pawn_premotion_queen{
                ...
                board.full_board[new_row][new_col]=Piece(900, map_piece_id_to_kind(new_piece), Team::White)
                board.full_board[new_row][new_col]=Piece(0, PieceId::empty, Team::None)
        ...
        if new_indexes=99{
            board.full_board[0][4]=Piece{0, PieceId::Empty, Team::None};
            board.full_board[0][7]=Piece{0, PieceId::Empty, Team::None};
            board.full_board[0][6]=Piece{0, PieceId::King, Team::Black};
            board.full_board[0][5]=Piece{500, PieceId::Rook, Team::Black};
        ...
        if new_indexes=99{
            board.full_board[0][1]=Piece{0, PieceId::Empty, Team::None};
            board.full_board[0][0]=Piece{0, PieceId::Empty, Team::None};
            board.full_board[0][4]=Piece{0, PieceId::Empty, Team::None};
            board.full_board[0][2]=Piece{0, PieceId::King, Team::Black};
            board.full_board[0][3]=Piece{500, PieceId::Rook, Team::Black};
        ...
        let old_points=board.full_board[new_row][new_col];
        if map_piece_id_to_kind(move_piece)=Kind::Pawn && new_col != old_col && old_points==0{
            ep_black((&mut board:Board, move_piece:PieceId, indexes:i32));
            return
        }
        if map_piece_id_to_kind move_piece==Kind::Pawn && (new_row-old_row).abs()==2{
